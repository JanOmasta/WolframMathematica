...
Do[   
  x[i+1]=x[i]+h;        
  y[i+1]=y[i]+3/2*h*f[x[i],y[i]]-1/2*h*f[x[i-1],y[i-1]],{i,1,pocetiteracii}];

TableForm[Table[{i,NumberForm[x[i],5],NumberForm[y[i],5]},{i,0,pocetiteracii}],
 	TableHeadings->{None,{"i","xi","yi"}},
 	TableSpacing->{1,3}]

AB=Table[{x[n],y[n]},{n,0,pocetiteracii}];
obrAB=ListPlot[AB,PlotStyle->PointSize[0.015]];
Show[gp,obrAB,PlotRange->{0,1.5}]
